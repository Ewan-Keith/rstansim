...
test_fit <- rstan::stan(file = "simple_reg_model.stan", data = test_data, cores = core_num)
```
...
```{R}
high_rhats <- extract_data(reg_study_col, 
                           estimates = "Rhat", 
...
The second hypothesis concerned the total time taken to fit a model, at a very high level we can check this by extracting the total time taken to carry out each `fit_models()` call.

```{R}
# simulation time for simple model on uncorrelated data
reg_study_col$simulations$`simple - uncorrelated simulation`$end_time -
  reg_study_col$simulations$`simple - uncorrelated simulation`$start_time

# simulation time for QR model on uncorrelated data
reg_study_col$simulations$`QR - uncorrelated simulation`$end_time -
  reg_study_col$simulations$`QR - uncorrelated simulation`$start_time

# simulation time for simple model on correlated data
reg_study_col$simulations$`simple - correlated simulation`$end_time -
  reg_study_col$simulations$`simple - correlated simulation`$start_time

# simulation time for QR model on correlated data
reg_study_col$simulations$`QR - correlated simulation`$end_time -
  reg_study_col$simulations$`QR - correlated simulation`$start_time
